...
<pre>
    ...
    */
    ...
    vertex_set neighbors(const vertex& a)const
    {
        return in_neighbors(a) + out_neighbors(a);
        //vertex_set result(in_neighbors(a));
        //const vertex_set& out = out_neighbors(a);
        //result.insert(out.begin(), out.end());
        //return result;
    }
  ...
  */
   ...
   bool includes_elm(const vertex_set& vs,const vertex&v) const{
       return vs.count(v);
   }
...
void absorb(const vertex &a, const vertex &b)
{
    if (a == b)
      return ;
...
/**
    dfs_search(v,depth,visited):   find vertex within distance 'depth' from v
*/
void dfs_search(const vertex& v,size_t depth, vertex_set &visited) const
{
    visited.insert(v);
    if (depth != 0)
    {
